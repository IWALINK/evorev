...
export function EventsList({ events, featuredEvent }: EventsListProps) {
  if (events.length === 0) {
    return (
    ...
    <>
      {events.map((event) => (
        <EventCard
          key={event.id}
          event={event}
          isFeatured={event.slug === featuredEvent?.slug}
        />
      ))}
    </>
  );
...
export function EventsSection({ children }: EventsSectionProps) {
  return (
    <section
      id="events"
